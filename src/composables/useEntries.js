...
export function useEntries() {
    ...
    const { user } = useAuth();
    const entries = ref([]);
    const entriesWithCategories = ref([]);
    ...
    const fetchEntriesWithCategories = async () => {
        await fetchEntries();
        const entriesWithCategoryInfo = await Promise.all(entries.value.map(async (entry) => {
    ...
    const fetchLatestEntries = async (limit = 4, lastDocId) => {
        try {
    ...
    const getEntriesByAuthor = (authorId) => {
        return entries.value.filter(entry => entry.authorId === authorId);
    };
    // Obtener entradas por categoría
    const fetchEntriesByCategory = async (categoryId) => {
        const result = await getCollection('entries', [
