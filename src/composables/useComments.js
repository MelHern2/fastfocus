...
export function useComments() {
    const { loading, error, getCollection, getDocument, addDocument, updateDocument, deleteDocument, subscribeToCollection } = useFirestore();
    const { user } = useAuth();
    ...
    const fetchCommentsWithReplies = async (entryId) => {
                ...
                if (data.parentId) {
                    try {
                        ...
                        if (parentComment && parentComment.authorId !== user.value.uid) {
                            ...
                            await createReplyNotification(parentComment.authorId, parentComment.authorName || parentComment.authorEmail, parentComment.authorEmail, docId, data.parentId, data.entryId, entryTitle, data.content);
        ...
        if (success) {
            ...
            if (!currentLikes.includes(userId) && newLikes.includes(userId)) {
                try {
                    ...
                    const entryTitle = entryDoc?.title || 'Entrada sin título';
                    await createLikeNotification(comment.authorId, comment.authorName || comment.authorEmail, comment.authorEmail, commentId, comment.entryId, entryTitle);
                }
                catch (notificationError) {
        ...
        if (success) {
            ...
            if (!currentDislikes.includes(userId) && newDislikes.includes(userId)) {
                try {
                    ...
                    const entryDoc = await getDocument('entries', comment.entryId);
                    const entryTitle = entryDoc?.title || 'Entrada sin título';
                    await createDislikeNotification(comment.authorId, comment.authorName || comment.authorEmail, comment.authorEmail, commentId, comment.entryId, entryTitle);
                }
                catch (notificationError) {
    ...
    const getEditTimeRemaining = (comment) => {
        if (!user.value || comment.authorId !== user.value.uid) {
            return 0;
        }
        const now = new Date();
    ...
    const getDeleteTimeRemaining = (comment) => {
        if (!user.value || comment.authorId !== user.value.uid) {
            return 0;
        }
        const now = new Date();
    ...
    const subscribeToComments = (entryId, callback) => {
