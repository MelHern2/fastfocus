import { ref } from 'vue';
import { collection, doc, getDocs, getDoc, addDoc, updateDoc, deleteDoc, query, where, orderBy, limit, startAfter, onSnapshot } from 'firebase/firestore';
import { db } from '@/firebase/config';
export function useFirestore() {
    const loading = ref(false);
    const error = ref(null);
    // Obtener todos los documentos de una colección con filtros y paginación
    const getCollection = async (collectionName, constraints, orderByField, orderDirection = 'desc', limitCount, lastDoc) => {
    ...
    const subscribeToCollection = (collectionName, callback, conditions, orderByField, limitCount) => {
        let q = collection(db, collectionName);
        if (conditions) {
